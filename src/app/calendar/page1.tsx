...
export default function Page() {
  ...
  const dateList = generateDateList(42 * count);
  ...
  useEffect(() => {
    setHeadTitle(showHeaderTitle(0));
    scrollRef.current?.addEventListener('scroll', onReachEnd);
    // setHeadTitle(showHeaderTitle((e.target! as HTMLElement).scrollTop));

    return () => {
      scrollRef.current?.removeEventListener('scroll', onReachEnd);
      lock.current = false;
    };
  }, [count]);

  function onReachEnd(e: Event) {
    const scrollTop = (e.target! as HTMLElement).scrollTop;
    console.log((e.target! as HTMLElement).scrollTop, 'scrollTopscrollTop');
    setHeadTitle(showHeaderTitle(scrollTop));
    if (scrollTop >= (count - 1) * 480 - DELTA_Y && !lock.current) {
      lock.current = true;
      console.log('get into the bottom', count, dateList.length);
  ...
  function generateDateList1(count: number): any[] {
    ...
    console.log(6 * count, '6 * count6 * count');
    return Array.from({ length: count / 7 }).map((_, index) => data.slice(index * 7, (index + 1) * 7));
  }
