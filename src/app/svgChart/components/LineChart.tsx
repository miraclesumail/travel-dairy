...
const LineChart = ({
  ...
  const maximumXFromData = Math.max(...data.map((e: any) => e.x));
  const maximumYFromData = Math.max(...data.map((e: any) => e.y));
  ...
  const LabelsYAxis = () => {
    ...
    return new Array(PARTS + 1).fill(0).map((_, index) => {
      ...
      return (
        ...
        >
          {parseFloat(maximumYFromData * (index / PARTS) as any).toFixed(precision)}
        </text>
      );
    });
  };
